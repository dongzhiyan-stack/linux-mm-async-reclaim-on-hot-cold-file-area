...
void f2fs_clear_page_cache_dirty_tag_for_file_area(struct folio *folio)
{
	struct address_space *mapping = folio->mapping;
	unsigned long flags;
	...
	//__xa_clear_mark(&mapping->i_pages, folio->index,PAGECACHE_TAG_DIRTY);
...
void f2fs_clear_page_cache_dirty_tag(struct folio *folio)
{
	...
	if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping)){
		return f2fs_clear_page_cache_dirty_tag_for_file_area(folio);
	}
#endif	
	xa_lock_irqsave(&mapping->i_pages, flags);
