...
#ifdef ASYNC_MEMORY_RECLAIM_IN_KERNEL
bool xa_get_mark_for_file_area(struct address_space *mapping, unsigned long index)
{
	if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping)){
		...
		/*仿照xa_get_mark() 这里不加锁，但是rcu_read_lock()防护遍历xarray tree时xa_node被释放了*/
		//xa_lock_irqsave(&mapping->i_pages, flags);
		rcu_read_lock();
		p_file_area = xas_load(&xas);
		if(is_file_area_entry(p_file_area) && !xa_is_value(p_file_area)){
			...
			printk("%s mapping:0x%llx index:%ld dirty\n",__func__,(u64)mapping,index);
		}
	...
	return xa_get_mark(&mapping->i_pages, index, PAGECACHE_TAG_DIRTY);
}
#endif
...
#ifdef ASYNC_MEMORY_RECLAIM_IN_KERNEL
		if (blkaddr == NEW_ADDR &&
		    xa_get_mark_for_file_area(mapping, index))
			return true;
#else		
		if (blkaddr == NEW_ADDR &&
