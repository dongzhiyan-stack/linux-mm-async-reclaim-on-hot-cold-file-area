...
void replace_page_cache_folio(struct folio *old, struct folio *new)
{
	...
	if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping)){
		//smp_rmb();
		//if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping))
			return replace_page_cache_folio_for_file_area(old,new);
	}
#endif
...
find_file_area:
	if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping)){
		//smp_rmb();
		//if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping))
			return filemap_get_folios_for_file_area(mapping,start,end,fbatch);
	}
#endif
...
find_file_area:
	if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping)){
		//smp_rmb();
		//if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping))
			return filemap_get_folios_contig_for_file_area(mapping,start,end,fbatch);
	}
#endif	
	rcu_read_lock();
...
find_file_area:
	if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping)){
		//smp_rmb();
		//if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping))
			return filemap_get_folios_tag_for_file_area(mapping,start,end,tag,fbatch);
	}
#endif
...
static loff_t mapping_seek_hole_data_for_file_area(struct address_space *mapping, loff_t start,
...
static vm_fault_t filemap_map_pages_for_file_area(struct vm_fault *vmf,
			     pgoff_t start_pgoff, pgoff_t end_pgoff)
{
	...
	do {
		...
		nr_pages = min(end, end_pgoff) - folio_index_for_xa_index + 1;
		...
		if (!folio_test_large(folio))
			...
			ret |= filemap_map_folio_range(vmf, folio,
					/*xas.xa_index*/folio_index_for_xa_index - folio->index, addr,
					nr_pages, &mmap_miss);
