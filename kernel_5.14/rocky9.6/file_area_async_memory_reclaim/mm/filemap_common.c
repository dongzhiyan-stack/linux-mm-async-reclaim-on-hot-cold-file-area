...
15:filemap_get_folios_contig_for_file_area
16:filemap_get_folios_tag_for_file_area
...
void replace_page_cache_folio_for_file_area(struct folio *old, struct folio *new)
{
	...
	if(old != rcu_dereference(p_file_area->pages[page_offset_in_file_area])){
		...
		pgoff_t end, struct folio_batch *fbatch, pgoff_t *indices)
{
	...
	while ((folio = find_get_entry_for_file_area(&xas, end, XA_PRESENT,&p_file_area,&page_offset_in_file_area,mapping))) {
		...
		//*start = base + nr;
		//indices[fbatch->nr] = xas.xa_index + page_offset_in_file_area;
		//indices[fbatch->nr] = folio->index;
...
unsigned filemap_get_folios_for_file_area(struct address_space *mapping, pgoff_t *start,
...
EXPORT_SYMBOL(filemap_get_folios_for_file_area);
