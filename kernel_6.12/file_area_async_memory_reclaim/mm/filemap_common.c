		...
		pgoff_t end, struct folio_batch *fbatch, pgoff_t *indices)
{
	...
	if (folio_batch_count(fbatch)) {
		...
		if (!xa_is_value(folio))
			...
			BUG();
			//nr = 1 << xa_get_order(&mapping->i_pages, indices[idx]); 这是file_area的索引，使用folio的索引才正确
...
*/
static int __folio_migrate_mapping_for_file_area(struct address_space *mapping,
...
void __folio_start_writeback_for_file_area(struct folio *folio, bool keep_write)
{
	long nr = folio_nr_pages(folio);
	struct address_space *mapping = folio_mapping(folio);
	int access_ret;
