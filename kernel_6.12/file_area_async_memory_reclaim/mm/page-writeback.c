...
static void tag_pages_for_writeback_for_file_area(struct address_space *mapping,
...
#ifdef ASYNC_MEMORY_RECLAIM_IN_KERNEL
static void __folio_mark_dirty_for_file_area(struct folio *folio, struct address_space *mapping,
		int warn)
{
	unsigned long flags;
    struct file_stat_base *p_file_stat_base;
...
#ifdef ASYNC_MEMORY_RECLAIM_IN_KERNEL
static bool __folio_end_writeback_for_file_area(struct folio *folio)
{
	long nr = folio_nr_pages(folio);
	struct address_space *mapping = folio_mapping(folio);
	bool ret;
...
#ifdef ASYNC_MEMORY_RECLAIM_IN_KERNEL
static void __folio_start_writeback_for_file_area(struct folio *folio, bool keep_write)
{
	long nr = folio_nr_pages(folio);
	struct address_space *mapping = folio_mapping(folio);
	int access_ret;
